...
import arek.inzynierka.model.*;
import arek.inzynierka.repository.*;
import arek.inzynierka.user.UserService;
import arek.inzynierka.utilities.UserUtilities;
import arek.inzynierka.validators.AwardValidator;
...
public class AwardPageController {
    ...
    public String changeAwardAction(@PathVariable("numberid") int numberid,Awards awards, BindingResult result, Model model, Locale locale) {
        ...
        System.out.println(awards1.getId()+"hjahahah");
        new AwardValidator().validate(awards1, result);
        if (result.hasErrors()) {
            returnPage = "editaward/"+awards1.getId();
        } else {
